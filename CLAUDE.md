...
When shipping a new `sd.X` primitive, the same commit MUST add `"X"` to the `known` set in `StackTemplates.swift`'s `StackDoctor` block (around line 250). Missed twice in a row before this rule; the doctor then warns "unknown permission — typo?" on legitimate stacks. The feature commit's staged set must include: new `Sources/DataSources/<Foo>.swift`, the `Sources/Bridge.swift` registration, `Sources/Channels.swift` (the channel registry — drives both `replayState()` and the JS template engine's `__sdSignalPaths`; see "Channel registry" below), `Runtime/api.js` (the `sd.X` namespace), `build.sh` if new sources/frameworks, `Sources/StackTemplates.swift` (the easily-forgotten one), and the test.
