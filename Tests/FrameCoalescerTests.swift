...
func registerFrameCoalescerTests() {
    ...
    // FrameBangReads — the off-main read schedule for dispatched bangs.
    // One AX frame read in flight per key; a dispatch that lands while one
    ...
    test("reads: resolving delivers and chains the LAST parked source") {
        ...
        try expect(next!.generation != first.generation, "a chained read gets its own generation")
    }
