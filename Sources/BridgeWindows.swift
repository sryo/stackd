...
/// god-object breakup continuation (follow-up to A1+A4+A5). Twenty-six
/// entries covering the focused-window + per-windowID + transactional
/// batch surface:
///
...
///
///   - `windows.byId.{setFrame, setFrameProbed, minimize, fullscreen,
///     raise, focus, close, frame, cornerHints, buttonFrames, info,
///     title, role, subrole, isMinimized, isFullscreen, hasToolbar,
///     isStandard, tabs, focusTab}` — per-CGWindowID surface. Each
///     `.ax` to satisfy the main-thread constraint; setters deny →
///     false, readers deny → null/empty per their canonical shape.
