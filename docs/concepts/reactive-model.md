...
| `sd.window.resizing` | `{ id, phase, frame, startFrame, edges }` (a user edge drag: `began` / `changed` per frame / `ended` on mouse-up; `edges` = `{ left, right, top, bottom }` moved since `startFrame`) |
