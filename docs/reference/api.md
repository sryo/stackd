...
- `sd.touchdevice` *(channel, event-driven, ~90Hz while touching)* — `{timestamp, frame, emittedAt, ageMs, synthetic?, touches:[{identifier, state, x, y, vx, vy, angle, size, pressure, majorAxis, minorAxis}]}`. x/y normalized 0..1 (bottom-left origin). Intermediate moves merge when the page lags; began/release frames never drop. A device silent for 120ms mid-touch gets a synthesized release (`synthetic: true`, empty `touches`). `ageMs` = hardware stamp → emit; event age on arrival = `ageMs + (performance.timeOrigin + performance.now() - emittedAt)`. `{ interval }` on subscribe is ignored. Zero CPU when unsubscribed.
