...
- `sd.touchdevice` *(channel, event-driven, ~90Hz while touching)* — `{timestamp, frame, device, emittedAt, ageMs, synthetic?, touches:[{identifier, state, x, y, vx, vy, angle, size, pressure, majorAxis, minorAxis}]}`. x/y normalized 0..1 (bottom-left origin). Every multitouch device is registered and re-registered across hot-plug, sleep/wake and unlock; `device` is its HID sender id (matches a scrollWheel tap's `senderId`). Intermediate moves merge when the page lags; began/release frames never drop. A device silent for 120ms mid-touch gets a synthesized release (`synthetic: true`, empty `touches`). `ageMs` = daemon receive → emit; event age on arrival = `ageMs + (performance.timeOrigin + performance.now() - emittedAt)`. `{ interval }` on subscribe is ignored. Zero CPU when unsubscribed.
...
- `setFrame(frame)` or `setFrame(id, frame, opts?) → Promise`. `opts:{duration?, easing?("easeOutCubic"|"linear"|"spring"), respectReduceMotion?(true)}` animates the id form on the daemon's display-link clock and resolves at settle. With the system Reduce Motion setting on, animations become an instant write unless `respectReduceMotion: false`. `opts.live: true` (no duration) sends the write through the app's AX writer off the main thread: a newer live write for the window replaces one still queued, there is no read-back, and the promise resolves when the write lands. Use it for frames that follow input, e.g. neighbors tracking `sd.windows.resizing`.
